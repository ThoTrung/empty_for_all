# -*- coding: utf-8 -*-
"""Stamp company_id on shared commercial partners when uniquely inferable."""
...
def migrate(cr, version):
    ...
    # res.company.partner_id → that company (unambiguous).
    cr.execute(
        """
    ...
    _logger.info(
        "rental 17.0.1.0.61: stamped company_id on %s shared partners; "
    ...
    for pid, name, companies in ambiguous[:50]:
        _logger.warning(
            "rental 17.0.1.0.61: partner %s (%s) linked to multiple companies %s — left shared",
            pid,
            name,
            companies,
        )
