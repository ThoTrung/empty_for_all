...
- Changes made:
  - `spa`: field `spa_is_order_discount_line` (SOL + AML); 2 wizard gán cờ + chặn khi SO đã có `out_invoice` posted; `sale.order.copy_data` bỏ dòng có cờ + reset `spa_points_redeemed`; helper phân bổ không lưu `spa/helper/order_discount.py`; `_reverse_moves` sửa `return` sai vị trí + thêm `_spa_rebalance_order_discount_lines()` (SO gốc làm chuẩn tỷ lệ); báo cáo `spa.revenue.report` phân bổ CK/điểm/voucher vào từng dòng SP (SQL window function), sửa `-ABS(...)` sai dấu; migration `1.0.4` (pre tạo cột, post backfill cờ theo dấu vết wizard, không dùng config param làm chính).
  - `spa_staff_payroll`: `_spa_commission_detail_vals_for_sale_order`/`_for_invoice` dùng helper phân bổ mới, skip mọi dòng `_spa_is_order_level_discount()`; xoá `_spa_global_discount_share_untaxed` cũ (vốn luôn trả 0 trên hoá đơn/credit note — bug có sẵn, nay fix theo); KPI + xếp hạng doanh số chuyển `_spa_recognized_amount_total()`/`amount_total_signed` → `_spa_recognized_amount_untaxed()`/`amount_untaxed_signed`; thêm field `order_discount_share` (cột "CK HD") trên commission line + report.
  - Test mới: `spa_staff_payroll/tests/test_spa_staff_payroll.py` (9 test: CK+điểm/voucher riêng và cùng lúc, danh mục cha có %, nhánh hoá đơn lẻ có CK, hoàn 1 phần/toàn bộ đơn dùng điểm); `spa/tests/test_spa_order_discount_line.py` (mới, 8 test: gán cờ, chặn sau khi có HĐ, nhân bản, báo cáo SP = báo cáo đơn); cập nhật test hiện có do đổi taxed→untaxed (`test_spa_staff_payroll.py` nhiều chỗ) và do báo cáo SP nay phân bổ CK (`spa/tests/test_spa_stats_reports.py::test_order_report_includes_ck_product_report_prorates`, đổi tên từ `..._excludes`).
- Files touched: xem [PAY-DEC-2026-09-16-01] mục Related files/modules.
- Validation done: chạy trên bản sao `drlai` (`drlai_test_loyalty`, đã xoá sau khi xong) — migration log xác nhận gắn cờ đúng 8 đơn thật + 5 dòng hoá đơn/credit note, 1 dòng cần soát tay. `--test-tags=/spa_staff_payroll:TestSpaStaffPayroll` → 52/52 pass. `--test-tags=/spa:TestSpaOrderDiscountLine,/spa:TestSpaStatsReports,/spa:TestSpaVoucherFlow,/spa:TestSpaLoyaltyLedgerCancel,/spa:TestSpaRevenueReport` → 41/41 pass. Chạy full `--test-tags=/spa,/spa_staff_payroll` 2 lần để rà soát: các fail/error còn lại (staff_readonly_group, vietqr, amount_total_for_point_recalc, card_sharing_upgrade — thiếu external ID CoA, customer_class_cron — cron quét dữ liệu KH thật trên bản sao, order_closed_by_returns — trùng số thứ tự journal entry với dữ liệu thật, pricelist_tier_block/product_required_staff_level — sai số làm tròn nhỏ) đều lặp lại giống nhau ở cả 2 lần chạy, không liên quan tới thay đổi lần này (đã đối chiếu diff, không đụng các file/logic liên quan) — kết luận là hệ quả của việc test trên bản sao dữ liệu thật, không phải hồi quy.
- Dependency impact check:
  - Dependents reviewed: `spa_card_upgrade_wizard.py` (sinh dòng `is_global_discount`, không đụng); `spa_treatment_card_report.py`, export bán hàng hằng ngày (`account_move.py:430,509`), giá trị nâng cấp thẻ — CHƯA trừ điểm/voucher, ghi nhận là giới hạn đã biết (xem mục Consequences ở DECISIONS); huỷ SO (`_spa_release_loyalty_ledger_on_cancel`) vẫn giữ dòng có cờ trên order đã huỷ — lỗi có sẵn, không thuộc phạm vi lần này.
  - Contract compatibility result: HH/KPI/doanh thu theo SP đổi số cho các đơn có CK toàn đơn hoặc điểm/voucher từ trước (nay net đúng); phiếu lương `done` không hồi tố.
  - Regression tests/manual checks run: xem Validation done.
- Open risks: hoá đơn gộp nhiều SO — tỷ lệ phân bổ ở nhánh SO (theo từng SO) và ở credit note rebalance (theo cả hoá đơn gốc) có thể lệch nếu các SO trong cùng hoá đơn có tỷ lệ giảm giá khác nhau; export bán hàng hằng ngày và báo cáo thẻ liệu trình chưa trừ điểm/voucher; 1 dòng dữ liệu cũ trên `drlai` (SO S52639) migration không tự gắn cờ được, cần soát tay.
- Next suggested steps: soát tay dòng S52639 trên `drlai` thật (không phải bản sao); cân nhắc bổ sung test cho trường hợp hoá đơn gộp nhiều SO nếu nghiệp vụ này xảy ra thường xuyên; xem xét trừ điểm/voucher trong export bán hàng hằng ngày nếu chủ dự án cần.
